...
item_sprites_craft = [[] for i in range(len(pos_order))]
...
for i in pos_order:
    for j in range(len(yaml_is_sexy['item_types'][i])):
        ...
        item_subsurface = pygame.transform.smoothscale(item_subsurface, (tuple(yaml_is_sexy['craft']['largesquares']['size'])))
        item_sprites_craft[pos_order.index(i)].append(item_subsurface)
...
def update_screen():
    ...
    """for i in range(len(yaml_is_sexy['craft']['text'])):
        screen_render.blit(texts[i], yaml_is_sexy['craft']['text'][text_list[i]]['positions'])""" #this is commented because texts arent properly done
    draw_items()
    outlines()
    counter()
...
def draw_items():
    for i in range(len(pos_order)):
        for j in range(len(type_dict[i])):
            try:
                qty_dict[type_dict[i][j+1]]
            except:
                pass
            else:
                if qty_dict[type_dict[i][j+1]] != 0:
                    screen_render.blit(item_sprites[i][j], tuple(yaml_is_sexy['craftstore']['smallsquares']['positions'][pos_order[i]][j]))
    for i in range(len(pos_order)):
        if item_qty[i] != 0:
            screen_render.blit(item_sprites_craft[i][item_type[i]-1], tuple(yaml_is_sexy['craft']['largesquares']['positions'][i]))
